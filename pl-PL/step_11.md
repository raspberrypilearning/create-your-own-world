...
Przełącz się na duszka `klucz`. Kliknij na `pokaż`{:class="block3looks"} w menu Skrypty, aby duszek pojawił się na scenie.

\--- /task \---

\--- task \---

\--- task \--- Zmień kostium duszka `klucz` tak, żeby był niebieski.
...
\--- task \--- Dodaj kod do duszka `klucz`, aby pojawiał się tylko w pokoju 3.
...
\--- task \--- Przetestuj swoją grę, aby sprawdzić, czy możesz zebrać duszka `klucz` i dodać go do swojego inwentarza.
...
![zrzut ekranu](images/world-door.png)

\--- /task \---

\--- task \---

\--- task \--- Dodaj kod do duszka `niebieskie drzwi`, aby był widoczny tylko w pokoju 3.
...
\--- task \--- Sprawdź swoją grę i zobacz, czy możesz zabrać niebieski klucz, aby otworzyć drzwi!
